...
function almostEqualsBtVector3(epsilon: number, u: Ammo.btVector3, v: Ammo.btVector3) {
  return Math.abs(u.x() - v.x()) < epsilon && Math.abs(u.y() - v.y()) < epsilon && Math.abs(u.z() - v.z()) < epsilon;
}
...
export default class Body {
  ...
  constructor(bodyConfig: BodyConfig, matrix: Matrix4, world: World) {
    ...
    this.emitCollisionEvents = bodyConfig.hasOwnProperty("emitCollisionEvents")
      ? bodyConfig.emitCollisionEvents
      : false;
